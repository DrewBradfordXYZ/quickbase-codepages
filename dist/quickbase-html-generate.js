...
export function generateQuickbaseHtml(config) {
    for (const [appIdentifier, appConfig] of Object.entries(config.apps)) {
        ...
        const quickbasePagesUrl = appConfig.quickbaseCodepagesUrl || "<-QUICKBASE-CODEPAGES-URL->";
        ...
        const commentsPageUrl = appConfig.quickbaseCodepagesUrl &&
        ...
        const commentsCssPageIds = appConfig.quickbaseCodepageCssIds &&
            appConfig.quickbaseCodepageCssIds.length > 0 &&
            !appConfig.quickbaseCodepageCssIds.includes("<-QUICKBASE-CODEPAGE-CSS-IDS->")
            ? ""
            : `    <!-- Set 'quickbaseCodepageCssIds' for "${appIdentifier}" in quickbase-config.js -->`;
        const commentsJsPageIds = appConfig.quickbaseCodepageJsIds &&
            appConfig.quickbaseCodepageJsIds.length > 0 &&
            !appConfig.quickbaseCodepageJsIds.includes("<-QUICKBASE-CODEPAGE-JS-IDS->")
            ? ""
